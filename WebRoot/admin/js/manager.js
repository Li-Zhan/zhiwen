$(function () {
	...
	$('#manager_add').dialog({         //弹出添加用户的对话框
		...
		buttons : [{
			...
			handler : function () {               //通过ajax提交要添加的数据
				if ($('#manager_add').form('validate')) {   //先判断是否验证通过
					$.ajax({
	...
	manager_tool = {                         //管理工具,定义点击按钮之后行为
		add : function () {                      //添加行为
			$('#manager_add').dialog('open');
			$('input[name="adminName"]').focus();
		},
		edit:function(){                    //修改行为
			var rows = $('#manager').datagrid('getSelections');     //判断选中的记录数，每次只允许修改一条记录
			if (rows.length > 1) {
		...
		search:function(){                    //查询行为
			...
			if(userName_find){
						$.ajax({
							...
							success : function (data) {
								if (data) {
									$('#manager').datagrid('selectRecord',data.uid);
									$('#manager').datagrid('loaded');
								}else{
									$('#manager').datagrid('loaded');
									$.messager.show({
